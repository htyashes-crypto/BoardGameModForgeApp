export interface CsprojInput {
  ...
  /**
   * Mod SDK 路径(绝对路径硬写进 `<ModSdkRoot>`;未配置(undefined / 空)时为空
   * 字符串 → Condition Exists 自动 fallback `$(UnityProjectRoot)Library\ScriptAssemblies\` + Warning)。
   */
  modSdkPath?: string;
}
...
/**
 * 生成 Mod 的 .csproj 文件内容(逐字移植旧模板,与 E 侧编译契约严格对齐)。
 *
 * **双模式(Condition Exists 自动判定)**:
 * - **本工程模式**(桌游编辑器开发期):SDK dll 走 `$(UnityProjectRoot)Library\ScriptAssemblies\`,
 *   UnityEngine dll 走 `$(UnityEditorInstallPath)Editor\Data\Managed\UnityEngine\`
 * - **Standalone 模式**(纯玩家 Mod 开发期):所有 dll 统一走 `$(ModSdkLibPath)`
 */
